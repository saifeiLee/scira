...
import { Mountain, Origami } from 'lucide-react';
...
const models = [
    ...
    },
    {
        ...
        icon: MistralIcon,
        iconClass: '!text-neutral-300',
        description: "Mistral's Small 3.1 model",
        color: 'orange',
        vision: true,
        experimental: false,
        category: 'Stable',
                                ...
                                >
                                    <button
                                        onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            console.log('切换到伴学丫模式');
                                            const newMode = 'banxueya';
                                            setSelectedGroup(newMode);

                                            // 通知消息
                                            const newModeText = '切换到伴学丫模式';
                                            const description = '伴学丫模式已开启';
                                            showSwitchNotification(newModeText, description);
                                        }}
                                        className={cn(
                                            'flex items-center gap-2 p-2 sm:px-3 h-8',
                                            'rounded-full transition-all duration-300',
                                            'border border-neutral-200 dark:border-neutral-800',
                                            'hover:shadow-md',
                                            selectedGroup === 'banxueya'
                                                ? 'bg-neutral-900 dark:bg-white text-white dark:text-neutral-900'
                                                : 'bg-white dark:bg-neutral-900 text-neutral-500',
                                        )}
                                    >
                                        <Origami className='h-3.5 w-3.5'/>
                                        <span className='hidden sm:block text-xs font-medium'>伴学丫</span>
                                    </button>
                                </div>
                            </div>
